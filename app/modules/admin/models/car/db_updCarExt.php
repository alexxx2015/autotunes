...
function db_updCarExt($p){	
	...
	//vextID
	if(isset($p[System_Properties::SQL_SET]['vextID'])){		
		$query .= 'vextID = "'.$db -> escape($p[System_Properties::SQL_SET]['vextID']).'", ';
	}
	...
	//WHERE
	$where = false;
	if (isset($p[System_Properties::SQL_WHERE]['carExtID'])){
		if ($where == false){
			$query .= ' WHERE ';
			$where = true;
		}else{
		...
		if (is_array($p[System_Properties::SQL_WHERE]['carExtID'])){
			$query .= ' (carExtID IN ( "'.implode('","', $db -> escape($p[System_Properties::SQL_WHERE]['carExtID'])).'") )';			
		}else{
			$query .= ' (carExtID = "'.$db -> escape($p[System_Properties::SQL_WHERE]['carExtID']).'") ';
		}
	}		
