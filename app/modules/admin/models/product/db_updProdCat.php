...
function db_updProdCat($p){	
	...
	//WHERE
	$where = false;
	if (isset($p[System_Properties::SQL_WHERE]['prodCatID'])){
		if ($where == false){
			$query .= ' WHERE ';
			$where = true;
		}else{
		...
		if (is_array($p[System_Properties::SQL_WHERE]['prodCatID'])){
			$query .= ' (prodCatID IN ( "'.implode('","', $db -> escape($p[System_Properties::SQL_WHERE]['prodCatID'])).'") )';			
		}else{
			$query .= ' (prodCatID = "'.$db -> escape($p[System_Properties::SQL_WHERE]['prodCatID']).'") ';
		}
	}		
