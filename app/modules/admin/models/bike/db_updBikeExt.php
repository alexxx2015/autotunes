...
function db_updBikeExt($p){	
	...
	//WHERE
	$where = false;
	if (isset($p[System_Properties::SQL_WHERE]['bikeExtID'])){
		if ($where == false){
			$query .= ' WHERE ';
			$where = true;
		}else{
		...
		$query .= ' (bikeExtID = "'.$db -> escape($p[System_Properties::SQL_WHERE]['bikeExtID']).'") ';
	}		
