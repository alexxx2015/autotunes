...
function db_selBikeAds($p){
	...
	//Add bikeBrand
	if (isset($p['bikeID'])){
		$query .= ' AND ( b.bikeID = '.$db -> escape($p['bikeID']).') ';
	}
	...
	//Add bikeAds
	if (isset($p['bikeAds']) && ($p['bikeAds'] > 0)){
		$p['bikeAds'] = $db -> escape($p['bikeAds']);
		$query .= ' AND (b.bikeAds = '.$p['bikeAds'].')';
	}
	...
	//Add bikeExt
	if (isset($p['bikeExt'])){
		$p['bikeExt'] = $db -> escape($p['bikeExt']);
		if(is_array($p['bikeExt'])){
			$bikeExt = implode(',', $p['bikeExt']);/*
			$query .= ' AND b.bikeID IN (SELECT DISTINCT be.bikeID
										FROM bikeExt AS ce
										WHERE be.vextID IN ('.$bikeExt.')) ';
			*/
			$query .= ' AND b.bikeID IN (SELECT be.bikeID
										FROM bikeExt AS be
										WHERE be.vextID IN ('.$bikeExt.')
										GROUP BY be.bikeID
										HAVING COUNT(be.bikeID) >= '.count($p['bikeExt']).'
										)';
			
			/*
			foreach ($p['bikeExt'] as $val) {
				$query .= ' AND EXISTS (SELECT be.bikeExtID
										FROM bikeExt AS ce
										WHERE 	(be.vextID = '.$val.')
												AND (be.bikeID = b.bikeID) 
										)';
			}
			*/
		}
