...
function db_selBikeCat($p=null){
	...
	$query = '	SELECT cc1.bikeCatID, cc1.lft, cc1.rgt, cc1.active, cc1.vcatID, COUNT(*)-1 AS level, ROUND((cc1.rgt - cc1.lft - 1) / 2) AS children
				FROM bikeCat AS cc1, bikeCat AS cc2
	...
	if (isset($p['bikeCatID'])){
		if(is_array($p['bikeCatID'])){
			$query .= ' AND (cc1.bikeCatID IN ("'.$db -> escape(implode('","',$p['bikeCatID'])).'")) ';
		}else{
			$query .= ' AND (cc1.bikeCatID = '.$db -> escape($p['bikeCatID']).') ';
		}
	}
