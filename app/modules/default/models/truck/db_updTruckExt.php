...
function db_updTruckExt($p){	
	...
	if(isset($p[System_Properties::SQL_SET]['vextID'])){		
		$query .= 'active = "'.$db -> escape($p[System_Properties::SQL_SET]['vextID']).'", ';
	}
	...
	//WHERE
	$where = false;
	if (isset($p[System_Properties::SQL_WHERE]['truckExtID'])){
		if ($where == false){
			$query .= ' WHERE ';
			$where = true;
		}else{
		...
		$query .= ' (truckExtID = "'.$db -> escape($p[System_Properties::SQL_WHERE]['truckExtID']).'") ';
	}		
