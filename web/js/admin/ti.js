...
function ti(){
	...
	this.init = function(){
		...
		$('#truckInsertForm input[name="truckDel"]').click(function(){
			$('#truckInsertForm').data('submit', 'truckDel');
		});
		
		$('#truckInsertForm input[name="truckSafe"]').click(function(){
			$('#truckInsertForm').data('submit', 'truckSafe');
		});
		
		$('#truckInsertForm').submit(function(p_ev){
			var _return = true;
			var _submit = $('#truckInsertForm').data('submit');
			if ((_submit != null) && (_submit == 'truckDel')){
				_return = confirm(g_arr["INFO_4"]);				
			}			
			else if ((_submit != null) && (_submit == 'truckSafe')){
	...
	this.init_upload_gal = function(){	
		...
		$('#photoGallery .delImg').click(function(p_ev){
			...
			$.getJSON(g_arr['adp']+'/truck/ajagfe', 
					{'pid':$_imgID},
					function(p_dat){
